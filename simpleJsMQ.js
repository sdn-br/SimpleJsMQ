 ...
 * @todo retry on callback error for queue and topic
...
let SimpleJsMQ = (function() {
	...
	class Payload {
		...
		constructor(event, type, data) {
			if (!(event instanceof Event)) {
				throw new ValueError(`Event expected for event, '${event && event.constructor ? event.constructor.name : typeof event} given`);
			}
	...
	class EventHandler {
		...
		/**
		 * constructor
		 * @param {String} [name=undefined] Name of the EventManger. Will be autogenerated if omitted. Must not contain numbers only
		 * @param {Object} [options={}]
		 * @param {EventBroker} [broker=undefined] EventBroker who will manage the new EventHandler Object
		 * @throws ValueError
		 * @constructor
		 * @public
		 */
		constructor(name = undefined, options = {}, broker = undefined) {
			if (name !== undefined && (typeof name !== 'string' || name.trim().length === 0 || name.trim().match(/^[0-9]+$/) !== null)) {
			...
			if (name === undefined) {
				name = `${this.constructor.name}_${this._id}`;
			} 	
		 ...
		 */
		_getDefaultOptions() {
			return EventHandler._defaultOptions;
		}
		...
		_setBroker(broker) {
			if (broker !== undefined && !(broker instanceof EventBroker)) {
				throw new ValueError(`EventBroker expected for broker, '${broker && broker.constructor ? broker.constructor.name : typeof broker} given`);
			}
			this._broker = broker;
		};
		 ...
		 * @param {String|Integer} subscriberName
		 * @param {Function} callback
		 * @throws ValueError
		 * @throws Error
		 * @public
